...
import PDFdesign from '@/components/Applicationtwo/PDFdesign';
...
const allapplications = () => {
  ...
  const [twopersoncvdata, setTwopersoncvdata] = useState(null);
  ...
  const fetchProfileData = async () => {
    try {
      ...
      if (!response.data || response.data.length === 0) {
        ...
        setCvdata(response.data.profile);
        setTwopersoncvdata(response.data.twoPersonApplication);
      }
    } catch (error) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex">
          {isEmpty ? (
            ...
            Array.isArray(twopersoncvdata) && twopersoncvdata.length > 0 ? (
              twopersoncvdata.map(profile => (
                <div key={profile._id} className="profile">
                 <PDFDownloadLink document={<PDFdesign profileData={profile} />} fileName="profile.pdf">
                  {({ loading }) => (
                    loading ? (
                      'Loading document...'
                    ) : (
